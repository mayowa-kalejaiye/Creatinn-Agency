      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div 
              key={index}
              className={`group relative overflow-hidden rounded-2xl bg-slate-100 cursor-pointer ${project.isDouble ? 'md:col-span-2' : ''}`}
            >
              {project.isDouble ? (
                // Combined Academy showcase with 2 images
              ...
              <div className={`absolute bottom-0 left-0 right-0 p-6 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ${project.isDouble ? '' : ''}`}>
